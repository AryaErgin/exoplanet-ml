...
def _deterministic_shuffle(df: pd.DataFrame, seed: int) -> pd.DataFrame:
    """Return a deterministically shuffled copy based on kepid-only hashing."""

    if df.empty:
        return df

    ids = df["kepid"].to_numpy(dtype=np.int64)
    # Linear-congruential style hash to provide a repeatable yet well-mixed order
    keys = (ids.astype(np.uint64) * np.uint64(6364136223846793005) + np.uint64(seed))
    keys ^= keys >> np.uint64(21)
    keys ^= keys << np.uint64(37)
    shuffled = df.assign(_order=keys)
    shuffled = shuffled.sort_values(["_order", "kepid"], kind="mergesort").reset_index(drop=True)
    return shuffled.drop(columns="_order")
...
def fetch_koi(n: int) -> List[int]:
    ...
    shuffled = _deterministic_shuffle(df, seed=42)
    return shuffled.head(min(n, len(shuffled)))["kepid"].tolist()


def fetch_nonplanets(n: int, exclude: Optional[Iterable[int]] = None) -> List[int]:
    NasaExoplanetArchive.ROW_LIMIT = -1
    tbl = NasaExoplanetArchive.query_criteria(
        table="q1_q17_dr25_sup_koi",
        select="kepid,koi_disposition",
    ...
    df["kepid"] = df["kepid"].astype(int)
    if exclude:
        exclude_set = {int(k) for k in exclude}
        if exclude_set:
...
if __name__ == "__main__":
    ...
    neg_ids = fetch_nonplanets(NEG_N, exclude=pos_ids)
